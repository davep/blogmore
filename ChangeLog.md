...
- Added automated social media cover image (Open Graph/Twitter Card)
  ...
  `auto_cover` frontmatter layout overrides.
...
- Added a `links dump` command line command to dump all external links found
...
- Added a "Top Internal Links" table to the stats page. When
  `with_backlinks` is enabled, the stats page now shows a table of up to 20
  posts ranked by the number of other posts that link to them.
  ([#406](https://github.com/davep/blogmore/pull/406))
...
- HTML files copied from the `extras` directory are no longer included in
  the generated `sitemap.xml`. Only pages generated by BlogMore appear in
  the sitemap.
  ([#384](https://github.com/davep/blogmore/pull/384))
- The stats page no longer carries a `noindex` robots directive.

...
class Linter:
    ...
    def lint(self) -> int:
        ...
        # sidebar_pages slugs must exist
        ...
        # links and socials in sidebar_config
            ...
            if link_type in sidebar and isinstance(sidebar[link_type], list):
                for item_dict in sidebar[link_type]:
                    if not isinstance(item_dict, dict):
                        continue
                    sidebar_url = item_dict.get("url")
                    if isinstance(sidebar_url, str):
                        self._check_config_link(
                            sidebar_url, f"Sidebar {link_type}", valid_urls
                        )
